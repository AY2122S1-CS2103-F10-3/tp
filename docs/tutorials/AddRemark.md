...
/**
 * Changes the remark of an existing friend in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the friend identified "
            + "by the index number used in the last friend listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the friend in the filtered friend list to edit the remark
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of friend data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the friend’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a friend.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.friend`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each friend.

Simply add the following to [`seedu.address.ui.FriendCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
``` java
public PersonCard(Person friend, int displayedIndex) {
    //...
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person friendToEdit = lastShownList.get(index.getZeroBased());
        Person editedFriend = new Person(
                friendToEdit.getName(), friendToEdit.getPhone(), friendToEdit.getEmail(),
                friendToEdit.getAddress(), remark, friendToEdit.getTags());

        model.setPerson(friendToEdit, editedFriend);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedFriend));
    }
     ...
     * {@code friendToEdit}.
     */
    private String generateSuccessMessage(Person friendToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, friendToEdit);
    }
```
